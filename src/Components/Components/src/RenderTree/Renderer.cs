    ...
    public abstract partial class Renderer : IDisposable
    {
        ...
        public virtual Task DispatchEventAsync(ulong eventHandlerId, EventFieldInfo fieldInfo, EventArgs eventArgs)
        {
            ...
            try
            {
                ...
                var timingRegion = TimingRegion.Impl.Open($"{nameof(Renderer)}.{nameof(DispatchEventAsync)} - invoke callback");
        ...
        {
            var timingRegion = TimingRegion.Impl.Open($"{nameof(Renderer)}.{nameof(ProcessRenderQueue)}");
            Dispatcher.AssertAccess();
            ...
            try
            {
                ...
                var updateDisplayRegion = TimingRegion.Impl.Open($"{nameof(Renderer)}.{nameof(ProcessRenderQueue)} - call UpdateDisplayAsync");
                updateDisplayTask = UpdateDisplayAsync(batch);
                TimingRegion.Impl.Close(updateDisplayRegion);
        ...
        {
            var timingRegion = TimingRegion.Impl.Open($"{nameof(Renderer)}.{nameof(RenderInExistingBatch)}");
